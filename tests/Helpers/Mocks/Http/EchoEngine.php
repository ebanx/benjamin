...
class EchoEngine extends Guzzle
{
    ...
    private function fakeResponse($url)
    {
        ...
        if ($this->responses[$endpoint]->json()['status'] === 'CONFLICT') {
            throw new ClientException('Conflict: invalid key', new Request('GET', $endpoint), new Response(409));
        }
        if ($this->responses[$endpoint]->json()['status'] === 'NOT FOUND') {
            throw new ClientException('Any other http status returned', new Request('GET', $endpoint), new Response(404));
        }
